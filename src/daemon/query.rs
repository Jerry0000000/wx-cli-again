...
pub async fn q_history(
    ...
    since: Option<i64>,
    until: Option<i64>,
    // 游标：create_time 严格小于该值（向更旧翻页）
    after_ts: Option<i64>,
    // 游标：create_time 严格大于该值（向更新翻页）
    ...
    // after_ts/before_ts 收紧时间窗，便于分片路由跳过无关库
    let eff_since = match (since, before_ts) {
        (Some(s), Some(b)) => Some(s.max(b.saturating_add(1))),
        (s, None) => s,
        (None, Some(b)) => Some(b.saturating_add(1)),
    };
    let eff_until = match (until, after_ts) {
        (Some(u), Some(a)) => Some(u.min(a.saturating_sub(1))),
        (u, None) => u,
        (None, Some(a)) => Some(a.saturating_sub(1)),
    };
    ...
    // 无时间窗/类型过滤时，可按分片 max_ts 做安全早停（见 history_can_early_stop）
    let allow_early_stop = since.is_none()
        && until.is_none()
        && before_ts.is_none()
        && after_ts.is_none()
        && msg_type.is_none();
    for (idx, shard) in shards.iter().enumerate() {
        let rel = shard.rel_key.clone();
        ...
        let since2 = eff_since;
        let until2 = eff_until;
        let after2 = after_ts;
        let before2 = before_ts;
        let limit2 = limit;
        let offset2 = offset;

        let opened = match db.open_query_conn(&rel).await? {
            Some(v) => v,
            None => continue,
        };
        let (conn, _mode) = opened;
        ...
        if allow_early_stop {
            ...
            all_msgs.truncate(needed.max(1));
            let top_min = all_msgs
                .iter()
                .filter_map(|m| m["timestamp"].as_i64())
                .min();
            let next_max = shards.get(idx + 1).map(|s| s.max_ts);
            if history_can_early_stop(needed, all_msgs.len(), top_min, next_max) {
                break;
            }
        }
    }
...
/// history 跨分片早停：堆已满 `needed` 且下一分片 max_ts 严格小于堆内最旧消息。
///
/// 若 `next_shard_max_ts` 为 None（无更多分片），可停。
/// 重叠时间窗时禁止「凑够条数就停」，否则会丢掉仍可能进入 top-N 的消息。
pub(crate) fn history_can_early_stop(
    needed: usize,
    collected_count: usize,
    collected_top_min_ts: Option<i64>,
    next_shard_max_ts: Option<i64>,
) -> bool {
    if collected_count < needed {
        return false;
    }
    let Some(min_ts) = collected_top_min_ts else {
        return false;
    };
    match next_shard_max_ts {
        None => true,
        Some(next_max) => next_max < min_ts,
    }
}

/// timeline 最多拉多少会话（按 last_timestamp 降序），避免 200×history。
pub(crate) fn timeline_session_cap(needed: usize) -> usize {
    needed.saturating_mul(2).clamp(8, 48)
}

/// 每个会话最多取几条（history limit）。
pub(crate) fn timeline_per_chat(needed: usize) -> usize {
    (needed / 2).clamp(10, 80)
}

/// 全局堆已满 `needed` 条，且剩余会话的 last_timestamp 都严格小于堆内最旧消息 → 可早停。
pub(crate) fn timeline_can_early_stop(
    needed: usize,
    collected_count: usize,
    top_needed_min_ts: Option<i64>,
    remaining_sessions_max_last_ts: i64,
) -> bool {
    if collected_count < needed {
        return false;
    }
    let Some(min_ts) = top_needed_min_ts else {
        return false;
    };
    // 更冷会话不可能再贡献进 top-needed（按 create_time 排序）
    remaining_sessions_max_last_ts < min_ts
}

/// 从已收集消息中取时间最新的 `needed` 条的最小 timestamp（用于早停判定）。
pub(crate) fn timeline_top_needed_min_ts(timestamps: &[i64], needed: usize) -> Option<i64> {
    if needed == 0 || timestamps.is_empty() {
        return None;
    }
    let mut ts: Vec<i64> = timestamps.to_vec();
    ts.sort_by_key(|t| std::cmp::Reverse(*t));
    if ts.len() < needed {
        return None;
    }
    ts.into_iter().take(needed).min()
}

/// 跨会话时间线：按时间合并多会话消息（适合日报 / Agent 记忆补全）。
pub async fn q_timeline(
    db: &DbCache,
    names: &Names,
    limit: usize,
    offset: usize,
    ...
    let needed = offset.saturating_add(limit).max(limit);
    let sess_cap = timeline_session_cap(needed);
    let per_chat = timeline_per_chat(needed);

    // 1) 取最近会话作候选（有 cap，不扫 200）
    let sess = q_sessions(db, names, sess_cap, false, false).await?;
    let mut sessions = sess
        .get("sessions")
    ...
    sessions.sort_by_key(|s| {
        std::cmp::Reverse(s.get("timestamp").and_then(|v| v.as_i64()).unwrap_or(0))
    });
    if sessions.len() > sess_cap {
        sessions.truncate(sess_cap);
    }

    let mut all: Vec<Value> = Vec::new();
    let mut sessions_queried = 0usize;

    for (idx, s) in sessions.iter().enumerate() {
        let uname = s.get("username").and_then(|v| v.as_str()).unwrap_or("");
        if uname.is_empty() {
            continue;
        }
        let last = s.get("timestamp").and_then(|v| v.as_i64()).unwrap_or(0);
        if let Some(s0) = since {
            if last > 0 && last < s0 {
                continue;
            }
        }
        if let Some(u0) = until {
            // session last 远早于 until 仍可能有消息落在窗内；仅跳过 last 明确晚于 until 之后全无
            let _ = u0;
        }
        ...
        .await
        {
            Ok(hist) => {
                ...
                let is_group = hist
                    ...
                    .unwrap_or(false);
                if let Some(msgs) = hist.get("messages").and_then(|m| m.as_array()) {
                    for m in msgs {
                        let mut row = m.clone();
                        if let Some(obj) = row.as_object_mut() {
                            obj.insert("chat".into(), json!(chat.clone()));
                            obj.insert("username".into(), json!(uname));
                            obj.insert("is_group".into(), json!(is_group));
                            obj.insert(
                                "chat_type".into(),
                                json!(chat_type_of(uname, names)),
                            );
                        }
                        all.push(row);
                    }
        ...
        // 早停：堆内 top-needed 已满，且更冷会话 last_ts 进不了堆
        let ts: Vec<i64> = all
            .iter()
            .filter_map(|m| m["timestamp"].as_i64())
            .collect();
        let top_min = timeline_top_needed_min_ts(&ts, needed);
        let remaining_max = sessions
            .iter()
            .skip(idx + 1)
            .filter_map(|s| s.get("timestamp").and_then(|v| v.as_i64()))
            .max()
            .unwrap_or(0);
        if timeline_can_early_stop(needed, ts.len(), top_min, remaining_max) {
            break;
        }
    }

    all.sort_by_key(|m| std::cmp::Reverse(m["timestamp"].as_i64().unwrap_or(0)));
    let paged: Vec<Value> = all.into_iter().skip(offset).take(limit).collect();
    let mut paged = paged;
    paged.sort_by_key(|m| m["timestamp"].as_i64().unwrap_or(0));
    ...
    if with_meta || debug_source {
        out["meta"] = json!({
            "source": "timeline",
            "sessions_considered": sessions.len(),
            "sessions_queried": sessions_queried,
            "session_cap": sess_cap,
            "per_chat": per_chat,
        });
    }
    Ok(out)
...
/// 通过 message_fts.db 的 `*_content` 表做关键词搜索（不依赖 MMFtsTokenizer）。
///
/// 列布局：c0=acontent, c1=message_local_id, c2=sort_seq, c3=local_type,
...
async fn search_via_fts(
    ...
    const FTS_REL: &str = "message/message_fts.db";
    if db.key_hex(FTS_REL).is_none() {
        return Ok(None);
    }
    let Some((conn, _mode)) = db.open_query_conn(FTS_REL).await? else {
        return Ok(None);
    };

    // 预先把 --in CHAT 解析成 username，在同一 conn 上查 name2id（勿二次 online-only open）
    let filter_unames: Option<Vec<String>> = if let Some(list) = chats {
        let mut u = Vec::new();
        for chat in list {
            if let Some(uname) = resolve_username(chat, names) {
                u.push(uname);
            }
        }
        if u.is_empty() {
    ...
    let results: Vec<Value> = tokio::task::spawn_blocking(move || {
        // name2id: rowid -> username 与 username -> rowid（session filter）
        let mut id2name: HashMap<i64, String> = HashMap::new();
        let mut name2id: HashMap<String, i64> = HashMap::new();
        {
            let mut stmt = conn.prepare("SELECT rowid, username FROM name2id")?;
            let rows = stmt.query_map([], |r| Ok((r.get::<_, i64>(0)?, r.get::<_, String>(1)?)))?;
            for row in rows.flatten() {
                name2id.insert(row.1.clone(), row.0);
                id2name.insert(row.0, row.1);
            }
        }

        let session_filter: Option<HashSet<i64>> = if let Some(ref unames) = filter_unames {
            let mut ids = HashSet::new();
            for uname in unames {
                if let Some(id) = name2id.get(uname) {
                    ids.insert(*id);
                }
            }
            if ids.is_empty() {
        ...
        let like = format!("%{}%", escape_like_pattern(&kw));
        let content_tables = [
        ...
        let mut hits: Vec<(i64, i64, i64, i64, i64, String)> = Vec::new();
        // (create_time, sort_seq, session_id, sender_id, local_type, snippet)

        // session filter 必须进 SQL：LIMIT 在 filter 之前会导致 --in CHAT 结果不全
        let session_ids: Option<Vec<i64>> =
            session_filter.as_ref().map(|s| s.iter().copied().collect());
        ...
        for table in content_tables {
            ...
            let mut clauses = vec!["c0 LIKE ? ESCAPE '\\'".to_string()];
            let mut params: Vec<Box<dyn rusqlite::types::ToSql>> = Vec::new();
            params.push(Box::new(like.clone()));
            if let Some(s) = since2 {
                clauses.push("c6 >= ?".into());
            ...
            if let Some(ref ids) = session_ids {
                ...
                clauses.push(format!("c4 IN ({})", ph.join(",")));
                for id in ids {
                    params.push(Box::new(*id));
                }
            }
            let sql = format!(
                "SELECT c0, c1, c2, c3, c4, c5, c6 FROM [{}] WHERE {} ORDER BY c6 DESC LIMIT ?",
                table,
                clauses.join(" AND ")
            );
            ...
            for row in rows.flatten() {
                let (snippet, _local_id, sort_seq, local_type, session_id, sender_id, create_time) =
                    row;
                hits.push((
                    create_time,
                    sort_seq,
        ...
        hits.truncate(limit2);

        let mut out = Vec::new();
        for (ts, _sort_seq, session_id, sender_id, local_type, snippet) in hits {
            let talker = id2name
                .get(&session_id)
                .cloned()
                .unwrap_or_else(|| format!("session:{}", session_id));
            let sender_u = id2name.get(&sender_id).cloned().unwrap_or_default();
            let display = names_map
                .get(&talker)
                .cloned()
                .unwrap_or_else(|| talker.clone());
            let sender_display = if sender_u.is_empty() {
                String::new()
            } else {
                names_map
                    .get(&sender_u)
                    .cloned()
                    .unwrap_or_else(|| sender_u.clone())
            };
            let mut row = json!({
                "timestamp": ts,
            ...
            // FTS 片段无完整 XML 时 appmsg_type 可能缺失；type_code/type_id 仍可机读
            attach_type_fields(&mut row, local_type, &snippet);
            out.push(row);
        }
...
pub async fn q_search(
    ...
    // 优先走 message_fts.db 的 content 表（LIKE），避免解密/扫描所有 message_N
    // 自定义 tokenizer 的 MATCH 路径需要 MMFtsTokenizer，这里用 content 表兜底。
    if msg_type.is_none() {
        match search_via_fts(db, names, keyword, chats.as_ref(), limit, since, until).await {
            Ok(Some(mut v)) => {
                // Agent-first：与 history/new_messages 一样始终带完整 freshness meta，
                // 避免 FTS 快路径静默丢掉 unknown_shards / status。
                let unknown_shards = current_unknown_shards(db, names);
                let windowed = since.is_some() || until.is_some() || chats.is_some();
                let hit = v.get("count").and_then(|c| c.as_u64()).unwrap_or(0) as usize;
                let meta = meta_for_global_query(
                    ...
                    debug_source,
                    Some(HashMap::from([(
                        "message/message_fts.db".into(),
                        "online_or_cache".into(),
                    )])),
                    None,
                );
                if let Some(obj) = v.as_object_mut() {
                    let mut meta_v = serde_json::to_value(&meta).unwrap_or(json!({}));
                    if let Some(m) = meta_v.as_object_mut() {
                        m.insert("source".into(), json!("message_fts"));
                        m.insert("mode".into(), json!("fts_content_like"));
                    }
                    obj.insert("meta".into(), meta_v);
                }
                return Ok(v);
            }
            Ok(None) => {}
    ...
    if let Some(chat_names) = chats {
        for chat_name in &chat_names {
            if let Some(uname) = resolve_username(chat_name, names) {
                let (shards, _) = find_msg_shards(db, names, &uname, None, None).await?;
                for shard in shards {
        ...
        // 全局搜索回退：遍历所有消息 DB（优先 online open）
        for rel_key in &names.msg_db_keys {
            let opened = match db.open_query_conn(rel_key).await? {
                Some(v) => v,
                None => continue,
            };
            let (conn, mode) = opened;
            scanned_rel_keys.insert(rel_key.clone());
            cache_modes.insert(rel_key.clone(), mode.as_str().to_string());
            let path_label = db.source_path(rel_key).to_string_lossy().into_owned();
            shard_paths.insert(rel_key.clone(), path_label.clone());
            let md5_lookup = names.md5_to_uname.clone();
            let names_map = names.map.clone();
            let rel_key2 = rel_key.clone();
            let path_label2 = path_label.clone();
    ...
    for (db_path, table_list) in by_path {
        let Some(rel_key) = path_to_rel_key.get(&db_path).cloned() else {
            continue;
        };
        let Some((conn, _)) = db.open_query_conn(&rel_key).await? else {
            continue;
        };
        let kw2 = kw.clone();
        let since2 = since;
...
/// 定位 talker 所在消息分片。
///
/// 优化点（对齐 other-wx-cli / 用户反馈）：
/// 1. 加密源 mtime 降序：优先热分片，避免先解密冷库
/// 2. `shard-meta.json` **按表** 时间窗过滤：since/until 可跳过不重叠分片
/// 3. 打开后写回 per-table min/max create_time + Timestamp 表
async fn find_msg_shards(
    ...
    let mut meta_file = super::shard_meta::load();
    let ordered = super::shard_meta::sort_rel_keys_by_mtime(db.db_dir(), &names.msg_db_keys);

    let mut scanned = 0usize;
    let mut results: Vec<MessageShard> = Vec::new();
    let mut meta_dirty = false;

    for rel_key in &ordered {
        let src_path = db
            .db_dir()
            .join(rel_key.replace('/', std::path::MAIN_SEPARATOR_STR));
        let src_mtime = super::shard_meta::source_mtime_ns(&src_path);
        let entry = meta_file.shards.get(rel_key);
        if !super::shard_meta::may_overlap(entry, src_mtime, &table_name, since, until) {
            continue;
        }
        if entry
            .map(|e| e.missing_tables.iter().any(|t| t == &table_name))
            .unwrap_or(false)
            && entry
                .map(|e| e.source_mtime_ns == src_mtime || e.source_mtime_ns == 0)
                .unwrap_or(false)
        {
            // 上次确认该 talker 表不在此分片，且源文件未变
            continue;
        }
        ...
        scanned += 1;
        let (conn, cache_mode) = opened;
        let tname = table_name.clone();
        let stats: (bool, Option<i64>, Option<i64>, Option<i64>) =
            tokio::task::spawn_blocking(move || {
                let table_exists: bool = conn
                    ...
                    .is_ok();
                if !table_exists {
                    return Ok::<_, anyhow::Error>((false, None, None, None));
                }
                ...
                let min_ts: Option<i64> = conn
                    .query_row(
                        &format!("SELECT MIN(create_time) FROM [{}]", tname),
                        [],
                        |row| row.get(0),
                    )
                    .ok()
                    .flatten();
                let shard_start: Option<i64> = conn
                    .query_row("SELECT timestamp FROM Timestamp LIMIT 1", [], |row| {
                        row.get(0)
                    })
                    .ok();
                Ok((true, min_ts, max_ts, shard_start))
            })
            .await??;

        let (exists, min_ts, max_ts, shard_start) = stats;
        if !exists {
            let e = meta_file.shards.entry(rel_key.clone()).or_default();
            if !e.missing_tables.iter().any(|t| t == &table_name) {
                e.missing_tables.push(table_name.clone());
            }
            e.source_mtime_ns = src_mtime;
            meta_dirty = true;
            continue;
        }

        super::shard_meta::record_open(
            &mut meta_file,
            rel_key,
            &src_path,
            &table_name,
        ...
        // 打开后二次时间窗过滤（meta 可能首次建立）
        if let (Some(s), Some(hi)) = (since, max_ts) {
            if hi < s {
                continue;
            }
        }
        if let (Some(u), Some(lo)) = (until, min_ts.or(shard_start)) {
            if lo > u {
                continue;
            }
        }
    ...
    if meta_dirty {
        super::shard_meta::save(&meta_file);
    }
...
#[allow(dead_code)] // tests still open by path
fn query_messages(
...
fn query_messages_conn(
    ...
    since: Option<i64>,
    until: Option<i64>,
    // create_time < after_ts（向更旧翻页）
    after_ts: Option<i64>,
    // create_time > before_ts（向更新翻页）
    before_ts: Option<i64>,
    msg_type: Option<i64>,
    limit: usize,
    offset: usize,
    ...
    for (local_id, local_type, ts, real_sender_id, content_bytes, ct) in rows {
        ...
        attach_type_fields(&mut msg, local_type, &content);
...
/// WeChat `local_type` 低 32 位（高位常是 WCDB/业务 flag）。
pub fn type_code(t: i64) -> i64 {
    (t as u64 & 0xFFFFFFFF) as i64
}

/// Agent 稳定英文 slug（与 CLI `--type` 对齐）；未知类型为 `type_<code>`。
pub fn type_id(t: i64) -> String {
    match type_code(t) {
        1 => "text".into(),
        3 => "image".into(),
        34 => "voice".into(),
        ...
        47 => "sticker".into(),
        48 => "location".into(),
        49 => "appmsg".into(), // 链接/文件/引用/合并… 见 appmsg_type
        50 => "call".into(),
        10000 => "system".into(),
        10002 => "revoke".into(),
        n => format!("type_{}", n),
...
/// 从 appmsg XML 提取 `<type>` 子类型（57=引用, 6=文件, 5=链接…）。
pub fn appmsg_type_code(content: &str) -> Option<i64> {
    let xml = strip_group_prefix(content);
    if !xml.contains("<appmsg") {
        return None;
    }
    extract_xml_text(&xml, "type")
        .and_then(|s| s.trim().parse::<i64>().ok())
        .filter(|&n| n > 0)
}

/// 写入 Agent 友好的类型字段（不覆盖已有 `type` 显示名）。
pub fn attach_type_fields(msg: &mut Value, local_type: i64, content: &str) {
    let code = type_code(local_type);
    if let Some(obj) = msg.as_object_mut() {
        obj.insert("type_code".into(), json!(code));
        obj.insert("type_id".into(), json!(type_id(local_type)));
        if code == 49 {
            if let Some(at) = appmsg_type_code(content) {
                obj.insert("appmsg_type".into(), json!(at));
            }
        }
    }
}
...
#[cfg(test)]
mod type_fields_tests {
    use super::{appmsg_type_code, attach_type_fields, type_code, type_id};
    use serde_json::json;

    #[test]
    fn type_code_masks_high_bits() {
        // high flags set (common WCDB pattern)
        let raw = 1i64 | (1i64 << 32);
        assert_eq!(type_code(raw), 1);
        assert_eq!(type_id(raw), "text");
    }

    #[test]
    fn type_id_matches_cli_slugs() {
        assert_eq!(type_id(49), "appmsg");
        assert_eq!(type_id(10002), "revoke");
        assert_eq!(type_id(999), "type_999");
    }

    #[test]
    fn appmsg_type_from_xml() {
        let xml = r#"<msg><appmsg><type>57</type><title>hi</title></appmsg></msg>"#;
        assert_eq!(appmsg_type_code(xml), Some(57));
        assert_eq!(appmsg_type_code("plain text"), None);
    }
    ...
    fn attach_type_fields_sets_agent_keys() {
        ...
        assert_eq!(msg["type_id"], "appmsg");
        assert_eq!(msg["appmsg_type"], 6);
    }
}

#[cfg(test)]
mod timeline_helpers_tests {
    use super::{
        history_can_early_stop, timeline_can_early_stop, timeline_per_chat, timeline_session_cap,
        timeline_top_needed_min_ts,
    };

    #[test]
    fn history_early_stop_requires_next_shard_colder_than_heap_floor() {
        // needed=2, heap full, next shard max still hot → must NOT stop
        assert!(!history_can_early_stop(2, 2, Some(10), Some(99)));
        // next shard strictly colder → safe
        assert!(history_can_early_stop(2, 2, Some(100), Some(99)));
        // no more shards → stop
        assert!(history_can_early_stop(2, 2, Some(100), None));
        // heap not full → never
        assert!(!history_can_early_stop(3, 2, Some(100), Some(1)));
    }

    #[test]
    fn session_cap_bounds_and_scales_with_needed() {
        assert_eq!(timeline_session_cap(1), 8);
        assert_eq!(timeline_session_cap(10), 20);
        assert_eq!(timeline_session_cap(1000), 48);
        assert!(timeline_session_cap(50) <= 48);
        assert!(timeline_session_cap(50) >= 8);
    }

    #[test]
    fn per_chat_bounds() {
        assert_eq!(timeline_per_chat(1), 10);
        assert_eq!(timeline_per_chat(40), 20);
        assert_eq!(timeline_per_chat(1000), 80);
    }

    #[test]
    fn top_needed_min_ts_requires_full_heap() {
        let ts = [100, 90, 80, 70];
        assert_eq!(timeline_top_needed_min_ts(&ts, 3), Some(80));
        assert_eq!(timeline_top_needed_min_ts(&ts, 5), None);
        assert_eq!(timeline_top_needed_min_ts(&[], 1), None);
    }

    #[test]
    fn early_stop_only_when_heap_full_and_remaining_colder() {
        assert!(!timeline_can_early_stop(10, 5, Some(100), 50));
        assert!(!timeline_can_early_stop(10, 10, None, 50));
        // remaining session still as hot as heap min → cannot stop
        assert!(!timeline_can_early_stop(10, 10, Some(100), 100));
        // remaining strictly colder than heap floor → stop
        assert!(timeline_can_early_stop(10, 10, Some(100), 99));
    }
}
...
pub async fn q_members(db: &DbCache, names: &Names, chat: &str) -> Result<Value> {
    ...
    // 降级路径：从消息记录中聚合发言过的成员（必须 open_query_conn，path 是加密源）
    let (shards, _) = find_msg_shards(db, names, &username, None, None).await?;
    if shards.is_empty() {
        return Ok(json!({
            "chat": display,
            "username": username,
    ...
    for shard in &shards {
        let Some((conn, _)) = db.open_query_conn(&shard.rel_key).await? else {
            continue;
        };
        let tname = shard.table.clone();
        let uname = username.clone();
...
pub async fn q_new_messages(
    ...
    for (uname, _) in &changed {
        let since_ts = state
            ...
            .copied()
            .unwrap_or(fallback_ts);
        // 用 since 做分片时间路由，跳过冷库
        let since_for_route = Some(since_ts.saturating_add(1));
        let (shards, _) = find_msg_shards(db, names, uname, since_for_route, None).await?;
        if shards.is_empty() {
            continue;
        }
        for shard in &shards {
        ...
        for shard in &shards {
            ...
            let Some((conn, _)) = db.open_query_conn(&rel).await? else {
                eprintln!("[new-messages] open failed for {}", rel);
                continue;
            };
